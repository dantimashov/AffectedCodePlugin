...
public class AffectedPsiElementImpl implements AffectedPsiElement<AffectedClass> {
    ...
    private void addAffectedElementsByChangedLines(AffectedClass affectedClass) throws AffectedCodePluginException {
        ...
        Document doc = Optional.ofNullable(PsiDocumentManager.getInstance(project).getDocument(file)).orElseThrow(
                        () -> new AffectedCodePluginException(format("Not found a loaded into memory file of class with name %s", affectedClass.getFileName()))
        );
